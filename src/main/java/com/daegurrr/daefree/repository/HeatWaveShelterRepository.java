...
public interface HeatWaveShelterRepository extends JpaRepository<HeatWaveShelter, Long> {
    @Query("SELECT new com.daegurrr.daefree.dto.shelter.ShelterResponse$Position(h.id, h.latitude, h.longitude) FROM HeatWaveShelter h WHERE h.restType = :restType")
    List<ShelterResponse.Position> findByRestType(@Param("restType") String restType);
